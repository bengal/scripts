...
if [ "$event" = clean ]; then
    ...
    if [ "$h" = hosta.example.org ]; then
        check_iface_addr eth0 203.0.113.2
        check_xfrm_esp dir out src 203.0.113.2/32 dst 0.0.0.0/0
        ping_host 192.0.2.2
        check_dns_server 1.2.3.4
        check_dns_server 8.8.8.9
        check_dns_domain test.tld
    elif [ "$h" = hostb.example.org ]; then
        check_xfrm_esp dir out src 0.0.0.0/0 dst 203.0.113.2/32
        ping_host 203.0.113.2
    fi

...
# preferred). Clients can reach the IPv4 internet via CLAT.
#
...
[ "$addr/$plen" == 192.0.0.0/32 ] || fail "Expected address 192.0.0.0/32 on veth0, found $addr/$plen"

log_action "Check the default route"
r=$(ip -4 -j route  | jq -r '.[] | select(.dst == "default" ) | "\(.dev) \(.prefsrc) \(.via.family)"')
[ "$r" == "veth0 192.0.0.0 inet6" ] || fail "Wrong default IPv4 route $r"
...
echo $! > /tmp/tshark.pid
pkill -F /tmp/tshark.pid
if ! grep -E "20.0.0.1.*192.0.0.0.*ICMP 75 Destination unreachable \(Port unreachable\)" /tmp/tshark.log; then
